...
- 편집기 스타일 타임라인
  ...
  - 클립을 드래그해 순서를 바꿀 수 있습니다.
  - 같은 순서에서 앞 클립과 겹치게 드래그하면 겹친 구간은 디졸브 전환으로 렌더링됩니다.
  - 클립 시작 위치가 다른 클립보다 앞서 순서가 바뀌면 전환 없이 클립들이 빈틈 없이 다시 붙습니다.
...
5. 클립을 드래그해 순서를 바꾸거나, 앞 클립과 겹쳐 디졸브 전환 구간을 만듭니다.
6. 삭제할 클립을 선택하고 `Delete` 키 또는 삭제 버튼으로 삭제합니다.
7. 잔물결 삭제가 켜져 있으면 삭제된 구간만큼 뒤 클립이 앞으로 당겨져 이어집니다.
8. 렌더 메뉴에서 무인코딩 또는 인코딩 렌더를 선택해 결과물을 저장합니다.
